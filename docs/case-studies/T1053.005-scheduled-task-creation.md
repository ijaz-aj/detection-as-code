...
## Screenshots
- `13-t1053-audit-policy-gap.png` — `auditpol` showing `Other Object Access Events → No Auditing` (sensor gap, before).
- `14-t1053-taskcontent-ignored.png` — document panel: `_ignored: winlog.event_data.TaskContent` beside the visible XML (index gap diagnosed).
- `15-t1053-mapping-keyword-vs-wildcard.png` — field mapping: old `keyword`/`ignore_above:1024` vs new `wildcard` (root cause + fix).
- `16-t1053-rule6-catch.png` — Rule 6's original query catching the `spawn` task after the fix (payoff).
